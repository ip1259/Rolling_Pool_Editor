...
def draw_header(canvas: tk.Canvas, headers: List[str], width: int, height: int, theme) -> None:
    edges = _column_edges(width)
    canvas.create_rectangle(0, 0, width, height, fill=theme.header_bg, outline="")
    for index, header in enumerate(headers):
        left, right = edges[index], edges[index + 1]
        canvas.create_rectangle(left, 0, right, height, outline=theme.row_even_bg)
        canvas.create_text(
...
def draw_row(canvas: tk.Canvas, record, row_index: int, effect_name: str,
    ...
    values = (
        ...
        (f"{chance_float * 100:.2f}%", row_bg, theme.chance_text_color, "center"),
    )
    for index, (text, bg, color, anchor) in enumerate(values):
        left, right = edges[index], edges[index + 1]
        canvas.create_rectangle(left, y, right, y + height, fill=bg, outline=border, tags="row")
        canvas.create_text(
    ...
    canvas.create_rectangle(left, y, right, y + height, fill=row_bg, outline=border, tags="row")
    if is_locked:
